...
class VendingMachine
  ...
  # 自動販売機の在庫
  def stock
    @juice_stock.map { |juice_name, juice_info| "#{juice_name}: #{juice_info[:quantity]}" }.join(', ')
  end

  # 購入可能かどうか
  def purchase_available?(juice_name, suica)
    juice_info = @juice_stock[juice_name]

    # チャージ残高が足りないもしくは在庫がない
    return false if juice_info.nil?

    suica.current_balance >= juice_info[:price] && juice_info[:quantity].positive?
  end
  ...
  def purchase(juice_name, suica)
    ...
    # 売上を増やす
    @sales_amount += juice_info[:price]
    # チャージ残高をjuice分減らす
    suica.reduce_balance(juice_info[:price])
  end

  # 購入可能なドリンクのリスト
  def available_drinks
    @juice_stock.select { |juice_name| purchase_available?(juice_name, Suica.new) }.keys.join(', ')
  end
